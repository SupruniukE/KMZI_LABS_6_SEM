...
def encrypt_enigma(message, list_rotors, reflector, replacement_symbol='X',
    ...
    rotors_direct = set_rotors_initial_position(get_rotors(list_rotors), initial_position)
    rotors_reverse = {k: v for k, v in reversed(list(rotors_direct.items()))}

    rotors_direct_output = set_rotors_initial_position(get_rotors(list_rotors), initial_position)
    rotors_reverse_output = {k: v for k, v in reversed(list(rotors_direct.items()))}

    reflector = get_reflector(reflector)
    message = replace_non_occurring_characters_and_uppercase(message, replacement_symbol)
    ...
    for symbol in message:
        ...
        for key in reflector.keys():
            if ''.join(reflector[key]).index(symbol) % 2 == 1:
                symbol = reflector[key][''.join(reflector[key]).index(symbol) // 2][0]
            else:
                symbol = reflector[key][''.join(reflector[key]).index(symbol) // 2][1]
