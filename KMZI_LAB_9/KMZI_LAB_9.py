...
# 1. Генерация сверхвозрастающей последовательности
# Старший член последовательности - 100-битное число
# В простейших случаях z = 6 (base64) либо z = 8 (ASCII)
# 2. Вычислений нормальной последовательности (открытый ключ)
# 3. Зашифровывание сообщения
# 4. Расшифровывание сообщения
# 5. Оценка времени выполнения операция зашифровывания и расшифровывания
import time
...
def EncryptByPackingTheSatchel(public_key, message):
    message_2 = [(len(public_key) - len(bin(ord(i))[2:])) * '0' + bin(ord(i))[2:] for i in message]
    encrypt_message_10 = []
    encrypt_message = []
...
def main():
    ...
    message_2 = [[(len_sequence - len(bin(ord(i))[2:])) * '0' + bin(ord(i))[2:] for i in message]]
    ...
    encrypt_message_10, encrypt_message = EncryptByPackingTheSatchel(public_key, message)
    encrypt_time = time.time() - encrypt_time

    decrypt_time = time.time()
    decrypt_message_10, decrypt_message = DecryptByPackingTheSatchel(inverse_a, encrypt_message_10, n, sequence)
