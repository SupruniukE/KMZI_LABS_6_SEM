...
def expansion_to_80_blocks(blocks_16_32):
    ...
    for i in range(16, 80):
        blocks_80_32.append((blocks_80_32[i - 3] ^ blocks_80_32[i - 8] ^ blocks_80_32[i - 14] ^ blocks_80_32[i - 16]) << 1)
    return blocks_80_32


def sha_1(message: str, output_in_console: bool):
    message_binary = ''.join([(16 - len(bin(ord(i))[2:])) * '0' + bin(ord(i))[2:] for i in message])
    ...
    A_32 = (32 - len(bin((A + a) % (2 ** 32))[2:])) * '0' + bin((A + a) % (2 ** 32))[2:]
    B_32 = (32 - len(bin((B + b) % (2 ** 32))[2:])) * '0' + bin((B + b) % (2 ** 32))[2:]
    C_32 = (32 - len(bin((C + c) % (2 ** 32))[2:])) * '0' + bin((C + c) % (2 ** 32))[2:]
    D_32 = (32 - len(bin((D + d) % (2 ** 32))[2:])) * '0' + bin((D + d) % (2 ** 32))[2:]
    E_32 = (32 - len(bin((E + e) % (2 ** 32))[2:])) * '0' + bin((E + e) % (2 ** 32))[2:]

    digest_2 = A_32 + B_32 + C_32 + D_32 + E_32
    digest_16 = ''.join([hex(int(digest_2[i * 4: (i + 1) * 4], 2))[2:] for i in range(len(digest_2) // 4)]).upper()
...
def main():
    print(f'----- Задание 1 -----')
    message = read_file('Alica.txt', 100)
    sha_1(message, True)
    ...
    for i in range(10):
        ...
        message = read_file('Alica.txt', int(100 + i * 10))
        sha_1(message, False)
